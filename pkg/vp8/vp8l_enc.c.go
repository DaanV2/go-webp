...
End:
  if !ok {
    // C: stdlib.Memset(huffman_codes, 0, 5 * histogram_image_size * sizeof(*huffman_codes))
  }
  return ok
}
...
func WriteRiffHeader(/* const */ pic *picture.Picture, uint64 riff_size, uint64 vp8l_size) int {
  ...
  // C: return pic.writer(riff, sizeof(riff), pic)
}
...
// input.
func VP8LEncodeImage(/* const */ config *config.Config, /*const*/ picture *picture.Picture) int {
...
// C: #endif
  ...
  if picture.ExtraInfo != nil {
    mb_w := (width + 15) >> 4
    mb_h := (height + 15) >> 4
    // C: stdlib.Memset(picture.ExtraInfo, 0, mb_w * mb_h * sizeof(*picture.ExtraInfo))
  }
