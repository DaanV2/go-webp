...
func VP8LBuildHuffmanTable(root_table *huffman.HuffmanTables, root_bits int, code_lengths []int, code_lengths_size int) int {
	...
	if total_size == 0 || root_table == nil {
		return total_size
	}
	...
	if true {
		...
		// next *huffman.HuffmanTablesSegment = (*huffman.HuffmanTablesSegment)WebPSafeMalloc(1, sizeof(*next))
		...
		// therefore chosen (any other arbitrary value could be chosen).
		{
			...
			//   var next_start *huffman.HuffmanCode = (*huffman.HuffmanCode)WebPSafeMalloc(next_size, sizeof(*next_start))
			//   if (next_start == nil) {
			//     return 0
			//   }
...
func VP8LHuffmanTablesAllocate(size int, huffman_tables *huffman.HuffmanTables) int {
	...
	// Allocate root.
	{
		// var start *huffman.HuffmanCode = (*huffman.HuffmanCode)WebPSafeMalloc(size, sizeof(*root.start))
