...
func ClampBits(width, height int, bits int, min_bits int, max_bits int, image_size_max int) int {
	...
	for bits > min_bits && image_size == 1 {
			...
			for i = 0; i < kNumEntropyIx; i++ {
				...
				if (i != kPalette && i != kPaletteAndSpatial) || use_palette {
					...
					if use_palette && (i == kPalette || i == kPaletteAndSpatial) {
						...
						for sorting_method = 0; sorting_method < kPaletteSortingNum; sorting_method++ {
...
func EncoderInit( /* const */ enc *VP8LEncoder) int {
	...
	var i int
	if !VP8LHashChainInit(&enc.hash_chain, pix_cnt) {
		return 0
	}

	for i = 0; i < 4; i++ {
		VP8LBackwardRefsInit(&enc.refs[i], refs_block_size)
	}
...
func GetHuffBitLengthsAndCodes( /* const */ histogram_image *VP8LHistogramSet /*const*/, huffman_codes *huffman.HuffmanTreeCode) int {
	var i, k int
	ok := 0
	total_length_size := 0
	...
	var huff_tree *huffman.HuffmanTree = nil

	// Iterate over all histograms and get the aggregate number of codes used.
	for i = 0; i < histogram_image_size; i++ {
		var histo *VP8LHistogram = histogram_image.histograms[i]
		var codes *huffman.HuffmanTreeCode = &huffman_codes[5*i]
	...
	//   huff_tree = (*huffman.HuffmanTree)WebPSafeMalloc(uint64(3) * max_num_symbols, sizeof(*huff_tree))
...
End:
	if !ok {
		// C: stdlib.Memset(huffman_codes, 0, 5 * histogram_image_size * sizeof(*huffman.Huffman_codes))
	}
	return ok
}
...
func ClearHuffmanTreeIfOnlyOneSymbol( /* const */ huffman_code *huffman.HuffmanTreeCode) {
	var k int
	count := 0
...
func StoreHuffmanTreeToBitMask( /* const */ bw *VP8LBitWriter /* const */, tokens *huffman.HuffmanTreeToken /* const */, num_tokens int /* const */, huffman_code *huffman.HuffmanTreeCode) {
	var i int
...
// 'huff_tree' and 'tokens' are pre-alloacted buffers.
func StoreHuffmanCode( /* const */ bw *VP8LBitWriter /*const*/, huff_tree *huffman.HuffmanTree /*const*/, tokens *huffman.HuffmanTreeToken /*const*/, huffman_code *huffman.HuffmanTreeCode) {
	var i int
	count := 0
	var symbols [2]int = [2]int{0, 0}
	kMaxBits := 8
...
func WriteHuffmanCode( /* const */ bw *VP8LBitWriter /*const*/, code *huffman.HuffmanTreeCode, code_index int) {
	depth := code.code_lengths[code_index]
	symbol := code.codes[code_index]
	VP8LPutBits(bw, symbol, depth)
}

func WriteHuffmanCodeWithExtraBits(bw *VP8LBitWriter /*const*/, code *huffman.HuffmanTreeCode, code_index int, bits int, n_bits int) {
	depth := code.code_lengths[code_index]
	symbol := code.codes[code_index]
	VP8LPutBits(bw, (bits<<depth)|symbol, depth+n_bits)
}

func StoreImageToBitMask( /* const */ bw *VP8LBitWriter, width int, histo_bits int /*const*/, refs *VP8LBackwardRefs /*const*/, histogram_symbols *uint32 /*const*/, huffman_codes *huffman.HuffmanTreeCode /*const*/, pic *picture.Picture) int {
	...
	var codes *huffman.HuffmanTreeCode = huffman_codes + 5*histogram_ix
	...
	for VP8LRefsCursorOk(&c) {
		...
		if tile_x != (x&tile_mask) || tile_y != (y&tile_mask) {
			tile_x = x & tile_mask
			tile_y = y & tile_mask
			histogram_ix = (histogram_symbols[(y>>histo_bits)*histo_xsize+(x>>histo_bits)] >> 8) & 0xffff
...
func EncodeImageNoHuffman( /* const */ bw *VP8LBitWriter /*const*/, argb *uint32 /*const*/, hash_chain *VP8LHashChain /*const*/, refs_array *VP8LBackwardRefs, width, height int, quality int, low_effort int /*const*/, pic *picture.Picture, percent_range int /*const*/, percent *int) int {
	var i int
	max_tokens := 0
	var refs *VP8LBackwardRefs
	var tokens *huffman.HuffmanTreeToken = nil
	huffman_codes := [5]HuffmanTreeCode{{0, nil, nil}}
	histogram_symbols := [1]uint32{0} // only one tree, one symbol
	cache_bits := 0
	...
	for i = 0; i < 5; i++ {
		var codes *huffman.HuffmanTreeCode = &huffman_codes[i]
		if max_tokens < codes.num_symbols {
			max_tokens = codes.num_symbols
		}
	}

	//   tokens = (*huffman.HuffmanTreeToken)WebPSafeMalloc(max_tokens, sizeof(*tokens))
	//   if (tokens == nil) {
	//     pic.SetEncodingError(picture.ENC_ERROR_OUT_OF_MEMORY)
	//     goto Error
	//   }
...
func EncodeImageInternal(
	...
	var i, histogram_image_size uint32 = 0, 0
	bit_array_size := 0
	...
	var cache_bits_init, write_histogram_image int
	var bw_init VP8LBitWriter = *bw, bw_best
	...
	for sub_configs_idx = 0; sub_configs_idx < config.sub_configs_size; sub_configs_idx++ {
		var sub_config *CrunchSubConfig = &config.sub_configs[sub_configs_idx]
		...
		for i_cache = 0; i_cache < tenary.If(sub_config.do_no_cache, 2, 1); i_cache++ {
			...
			//   huffman_codes = (*huffman.HuffmanTreeCode)WebPSafeCalloc(bit_array_size, sizeof(*huffman.Huffman_codes))
...
func WriteImageSize( /* const */ pic *picture.Picture /*const*/, bw *VP8LBitWriter) int {
	width := pic.width - 1
	height := pic.height - 1
...
func MakeInputImageCopy( /* const */ enc *VP8LEncoder) int {
	var picture *picture.Picture = enc.pic
	width := picture.Width
...
func SearchColorGreedy( /* const */ palette []uint32, palette_size int, color uint32) uint32 {
	// C: (void)palette_size
...
func ApplyPalette( /* const */ src *uint32, src_stride uint32, dst *uint32, dst_stride uint32 /* const */, palette *uint32, palette_size, width, height, xbits int /* const */, pic *picture.Picture) int {
	...
	if palette_size < APPLY_PALETTE_GREEDY_MAX {
		...
		hash_functions := [3]func(uint32) uint32{
			ApplyPaletteHash0,
			ApplyPaletteHash1,
			ApplyPaletteHash2,
		}
		...
		for i = 0; i < 3; i++ {
			...
			// C: stdlib.Memset(buffer, 0xff, sizeof(buffer))
...
// C: #undef APPLY_PALETTE_FOR
// C: #undef PALETTE_INV_SIZE_BITS
// C: #undef PALETTE_INV_SIZE
// C: #undef APPLY_PALETTE_GREEDY_MAX
...
func VP8LEncoderNew( /* const */ config *config.Config /*const*/, pict *picture.Picture) *VP8LEncoder {
...
func VP8LEncoderDelete(enc *VP8LEncoder) {
	if enc != nil {
		...
		for i = 0; i < 4; i++ {
			VP8LBackwardRefsClear(&enc.refs[i])
		}
...
func EncodeStreamHook(input *StreamEncodeContext, data *void2) int {
	var params *StreamEncodeContext = input
	var config *config.Config = params.config
	var picture *picture.Picture = params.picture
	...
	for idx = 0; idx < num_crunch_configs; idx++ {
		...
		predictor_transform_bits := 0
		cross_color_transform_bits := 0
		...
		enc.use_predict = (entropy_idx == kSpatial) || (entropy_idx == kSpatialSubGreen) || (entropy_idx == kPaletteAndSpatial)
...
func VP8LEncodeStream( /* const */ config *config.Config /*const*/, picture *picture.Picture /*const*/, bw_main *VP8LBitWriter) int {
	var enc_main *VP8LEncoder = VP8LEncoderNew(config, picture)
	enc_side * VP8LEncoder = nil
	var crunch_configs CrunchConfig[CRUNCH_CONFIGS_MAX]
	var num_crunch_configs_main, num_crunch_configs_side int = 0, 0
	var idx int
	red_and_blue_always_zero := 0
	var worker_main, worker_side WebPWorker
	var params_main, params_side StreamEncodeContext
	// The main thread uses picture.stats, the side thread uses stats_side.
	var stats_side WebPAuxStats
	var bw_side VP8LBitWriter
	var picture_side picture.Picture
...
// C: #undef CRUNCH_SUBCONFIGS_MAX

// Encodes the picture.
// Returns 0 if config or picture is nil or picture doesn't have valid argb
// input.
func VP8LEncodeImage( /* const */ config *config.Config /*const*/, pict *picture.Picture) int {
	...
	if config == nil || pict.ARGB == nil {
		return pict.SetEncodingError(pict.ENC_ERROR_nil_PARAMETER)
	}
	...
	if !VP8LBitWriterInit(&bw, initial_size) {
		pict.SetEncodingError(pict.ENC_ERROR_OUT_OF_MEMORY)
		goto Error
	}

	if !WebPReportProgress(pict, 1, &percent) {
	UserAbort:
		pict.SetEncodingError(pict.ENC_ERROR_USER_ABORT)
		goto Error
	}
	// Reset stats (for pure lossless coding)
	if pict.stats != nil {
		var stats *WebPAuxStats = pict.stats
		// C: stdlib.Memset(stats, 0, sizeof(*stats))
		// stats.PSNR[0] = 99.0f
		// stats.PSNR[1] = 99.0f
		// stats.PSNR[2] = 99.0f
		// stats.PSNR[3] = 99.0f
	...
	// Write image size.
	if !WriteImageSize(pict, &bw) {
		pict.SetEncodingError(pict.ENC_ERROR_OUT_OF_MEMORY)
		goto Error
	}

	has_alpha = pict.WebPPictureHasTransparency(pict)
	...
	if !WebPReportProgress(pict, 2, &percent) {
		goto UserAbort
	}

	// Encode main image stream.
	if !VP8LEncodeStream(config, pict, &bw) {
		goto Error
	}

	if !WebPReportProgress(pict, 99, &percent) {
		goto UserAbort
	}

	// Finish the RIFF chunk.
	if !WriteImage(pict, &bw, &coded_size) {
		goto Error
	}

	if !WebPReportProgress(pict, 100, &percent) {
		goto UserAbort
	}

	if pict.ExtraInfo != nil {
		mb_w := (width + 15) >> 4
		mb_h := (height + 15) >> 4
		stdlib.Memset(pict.ExtraInfo, 0, mb_w*mb_h*sizeof(*picture.ExtraInfo))
	}
