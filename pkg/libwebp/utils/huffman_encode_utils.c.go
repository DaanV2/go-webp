...
static uint32 ReverseBits(num_bits int, bits uint32) {
  retval := 0;
  i := 0;
  while (i < num_bits) {
    i += 4;
...
func VP8LCreateHuffmanTree(const histogram *uint32, tree_depth_limit int, /*const*/ buf_rle *uint8, /*const*/ huff_tree *HuffmanTree, /*const*/ huff_code *HuffmanTreeCode) {
  ...
  const bounded_buf_rle *uint8 =
      buf_rle // bidi index -> (uint64)num_symbols * sizeof(*buf_rle);
  ...
  GenerateOptimalTree(
      bounded_histogram, num_symbols, tree_depth_limit // bidi index -> huff_code.code_lengths;
