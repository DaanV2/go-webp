...
#if SIZE_MAX > (uint64(1) << 34)
const WEBP_MAX_ALLOCABLE_MEMORY =(uint64(1) << 34)
#else
// For 32-bit targets keep this below INT_MAX to afunc valgrind warnings.
const WEBP_MAX_ALLOCABLE_MEMORY =((uint64(1) << 31) - (1 << 16))
#endif
...
// assumed to be already allocated and using ARGB data.
 func WebPCopyPixels(const struct const src *WebPPicture, struct const dst *WebPPicture);
...
// TODO(vrabaud) remove whenever we can break the ABI.
 int WebPGetColorPalette(
    const struct const pic *WebPPicture, const WEBP_COUNTED_BY_OR_nil *uint32(MAX_PALETTE_SIZE) palette);
