...
func ITransform_One_SSE2(/* const */ ref *uint8, /*const*/ in *int16, dst *uint8) {
  ...
  const __m128i in01 = _mm_loadu_si128((/* const */ __*m128i)&in[0]);
  const __m128i in23 = _mm_loadu_si128((/* const */ __*m128i)&in[8]);
  ...
  // Add inverse transform to 'ref' and store.
  {
    ...
    _mm_storeu_si128((__*m128i)buf, ref0123);
...
func ITransform_Two_SSE2(/* const */ ref *uint8, /*const*/ in *int16, dst *uint8) {
  // This implementation makes use of 16-bit fixed point versions of two
  // multiply constants:
  //    K1 = sqrt(2) * cos (pi/8) ~= 85627 / 2^16
  //    K2 = sqrt(2) * sin (pi/8) ~= 35468 / 2^16
  ...
  {
    const __m128i tmp0 = _mm_loadu_si128((/* const */ __*m128i)&in[0]);
    const __m128i tmp1 = _mm_loadu_si128((/* const */ __*m128i)&in[8]);
    const __m128i tmp2 = _mm_loadu_si128((/* const */ __*m128i)&in[16]);
    const __m128i tmp3 = _mm_loadu_si128((/* const */ __*m128i)&in[24]);
  ...
  // Add inverse transform to 'ref' and store.
  {
    ...
    ref0 = _mm_loadl_epi64((/* const */ __*m128i)&ref[0 * BPS]);
    ref1 = _mm_loadl_epi64((/* const */ __*m128i)&ref[1 * BPS]);
    ref2 = _mm_loadl_epi64((/* const */ __*m128i)&ref[2 * BPS]);
    ref3 = _mm_loadl_epi64((/* const */ __*m128i)&ref[3 * BPS]);
    // Convert to 16b.
    ...
    _mm_storel_epi64((__*m128i)&dst[0 * BPS], ref0);
    _mm_storel_epi64((__*m128i)&dst[1 * BPS], ref1);
    _mm_storel_epi64((__*m128i)&dst[2 * BPS], ref2);
    _mm_storel_epi64((__*m128i)&dst[3 * BPS], ref3);
  }
}

// Does one or two inverse transforms.
func ITransform_SSE2(/* const */ ref *uint8, /*const*/ in *int16, dst *uint8, do_two int) {
  if (do_two) {
...
func FTransformPass2_SSE2(/* const */ __const v *m128i01, /*const*/ __const v *m128i32, out *int16) {
  const __m128i zero = _mm_setzero_si128();
  ...
  _mm_storeu_si128((__*m128i)&out[0], d0_g1);
  _mm_storeu_si128((__*m128i)&out[8], d2_f3);
}

func FTransform_SSE2(/* const */ src *uint8, /*const*/ ref *uint8, out *int16) {
  const __m128i zero = _mm_setzero_si128();
  // Load src.
  const __m128i src0 = _mm_loadl_epi64((/* const */ __*m128i)&src[0 * BPS]);
  const __m128i src1 = _mm_loadl_epi64((/* const */ __*m128i)&src[1 * BPS]);
  const __m128i src2 = _mm_loadl_epi64((/* const */ __*m128i)&src[2 * BPS]);
  const __m128i src3 = _mm_loadl_epi64((/* const */ __*m128i)&src[3 * BPS]);
  // 00 01 02 03 *
  // 10 11 12 13 *
  // 20 21 22 23 *
  // 30 31 32 33 *
  ...
  // Load ref.
  const __m128i ref0 = _mm_loadl_epi64((/* const */ __*m128i)&ref[0 * BPS]);
  const __m128i ref1 = _mm_loadl_epi64((/* const */ __*m128i)&ref[1 * BPS]);
  const __m128i ref2 = _mm_loadl_epi64((/* const */ __*m128i)&ref[2 * BPS]);
  const __m128i ref3 = _mm_loadl_epi64((/* const */ __*m128i)&ref[3 * BPS]);
...
func FTransform2_SSE2(/* const */ src *uint8, /*const*/ ref *uint8, out *int16) {
  const __m128i zero = _mm_setzero_si128();
...
func FTransformWHTRow_SSE2(/* const */ /* const */ in *int16, __const out *m128i) {
  const __m128i kMult = _mm_set_epi16(-1, 1, -1, 1, 1, 1, 1, 1);
  const __m128i src0 = _mm_loadl_epi64((__*m128i)&in[0 * 16]);
  const __m128i src1 = _mm_loadl_epi64((__*m128i)&in[1 * 16]);
  const __m128i src2 = _mm_loadl_epi64((__*m128i)&in[2 * 16]);
  const __m128i src3 = _mm_loadl_epi64((__*m128i)&in[3 * 16]);
...
func FTransformWHT_SSE2(/* const */ in *int16, out *int16) {
  // Input is 12b signed.
...

  {
    ...
    _mm_storeu_si128((__*m128i)&out[0], _mm_srai_epi16(b0b1, 1));
    _mm_storeu_si128((__*m128i)&out[8], _mm_srai_epi16(b2b3, 1));
  }
}
...
func CollectHistogram_SSE2(/* const */ ref *uint8, /*const*/ pred *uint8, start_block int, end_block int, /* const */ histo *VP8Histogram) {
  const __m128i zero = _mm_setzero_si128();
...
static  func VE8uv_SSE2(dst *uint8, /*const*/ top *uint8) {
  var j int
...
static  func VE16_SSE2(dst *uint8, /*const*/ top *uint8) {
...
static  func VerticalPred_SSE2(dst *uint8, /*const*/ top *uint8, size int) {
  if (top != nil) {
    if (size == 8) {
      VE8uv_SSE2(dst, top);
    } else {
...
static  func HE8uv_SSE2(dst *uint8, /*const*/ left *uint8) {
  var j int
  for j = 0; j < 8; j++ {
...
static  func HE16_SSE2(dst *uint8, /*const*/ left *uint8) {
  var j int
  for j = 0; j < 16; j++ {
    const __m128i values = _mm_set1_epi8((byte)left[j]);
...
static  func HorizontalPred_SSE2(dst *uint8, /*const*/ left *uint8, size int) {
  if (left != nil) {
    if (size == 8) {
      HE8uv_SSE2(dst, left);
    } else {
...
static  func TM_SSE2(dst *uint8, /*const*/ left *uint8, /*const*/ top *uint8, size int) {
  const __m128i zero = _mm_setzero_si128();
  var y int
  if (size == 8) {
    const __m128i top_values = _mm_loadl_epi64((/* const */ __*m128i)top);
    ...
    for y = 0; y < 16; ++y, dst += BPS {
      ...
      _mm_store_si128((__*m128i)dst, out);
    }
  }
}

static  func TrueMotion_SSE2(dst *uint8, /*const*/ left *uint8, /*const*/ top *uint8, size int) {
  if (left != nil) {
    if (top != nil) {
...
static  func DC8uv_SSE2(dst *uint8, /*const*/ left *uint8, /*const*/ top *uint8) {
  const __m128i top_values = _mm_loadl_epi64((/* const */ __*m128i)top);
...
static  func DC8uvNoLeft_SSE2(dst *uint8, /*const*/ top *uint8) {
  const __m128i zero = _mm_setzero_si128();
  const __m128i top_values = _mm_loadl_epi64((/* const */ __*m128i)top);
...
static  func DC8uvNoTop_SSE2(dst *uint8, /*const*/ left *uint8) {
...
static  func DC8uvMode_SSE2(dst *uint8, /*const*/ left *uint8, /*const*/ top *uint8) {
  if (top != nil) {
...
static  func DC16_SSE2(dst *uint8, /*const*/ left *uint8, /*const*/ top *uint8) {
...
static  func DC16NoLeft_SSE2(dst *uint8, /*const*/ top *uint8) {
  const __m128i top_row = _mm_load_si128((/* const */ __*m128i)top);
  DC := VP8HorizontalAdd8b(&top_row) + 8;
  Put16_SSE2(DC >> 4, dst);
}

static  func DC16NoTop_SSE2(dst *uint8, /*const*/ left *uint8) {
  // 'left' is contiguous so we can reuse the top summation.
...
static  func DC16Mode_SSE2(dst *uint8, /*const*/ left *uint8, /*const*/ top *uint8) {
  if (top != nil) {
    if (left != nil) {  // top and left present
...
// vertical
static  func VE4_SSE2(dst *uint8, /*const*/ top *uint8) {
...
// horizontal
static  func HE4_SSE2(dst *uint8, /*const*/ top *uint8) {
  X := top[-1];
  I := top[-2];
  J := top[-3];
  K := top[-4];
...
static  func DC4_SSE2(dst *uint8, /*const*/ top *uint8) {
  dc := 4;
  var i int
...
// Down-Left
static  func LD4_SSE2(dst *uint8, /*const*/ top *uint8) {
  const __m128i one = _mm_set1_epi8(1);
...
// Vertical-Right
static  func VR4_SSE2(dst *uint8, /*const*/ top *uint8) {
  const __m128i one = _mm_set1_epi8(1);
  I := top[-2];
  J := top[-3];
  K := top[-4];
...
// Vertical-Left
static  func VL4_SSE2(dst *uint8, /*const*/ top *uint8) {
  const __m128i one = _mm_set1_epi8(1);
  const __m128i ABCDEFGH = _mm_loadl_epi64((/* const */ __*m128i)top);
...
// Down-right
static  func RD4_SSE2(dst *uint8, /*const*/ top *uint8) {
  const __m128i one = _mm_set1_epi8(1);
...
static  func HU4_SSE2(dst *uint8, /*const*/ top *uint8) {
  I := top[-2];
  J := top[-3];
  K := top[-4];
  L := top[-5];
...
static  func HD4_SSE2(dst *uint8, /*const*/ top *uint8) {
  X := top[-1];
  I := top[-2];
  J := top[-3];
  K := top[-4];
...
static  func TM4_SSE2(dst *uint8, /*const*/ top *uint8) {
  const __m128i zero = _mm_setzero_si128();
...
func Intra4Preds_SSE2(dst *uint8, /*const*/ top *uint8) {
...
func IntraChromaPreds_SSE2(dst *uint8, /*const*/ left *uint8, /*const*/ top *uint8) {
  // U block
...
func Intra16Preds_SSE2(dst *uint8, /*const*/ left *uint8, /*const*/ top *uint8) {
...
static  int SSE_16xN_SSE2(/* const */ a *uint8, /*const*/ b *uint8, num_pairs int) {
  ...
  for i = 0; i < num_pairs; i++ {
    ...
    const __m128i b1 = _mm_loadu_si128((/* const */ __*m128i)&b[BPS * 1]);
    __m128i sum1, sum2;
...
func SSE16x16_SSE2(/* const */ a *uint8, /*const*/ b *uint8) int {
  return SSE_16xN_SSE2(a, b, 8);
}

func SSE16x8_SSE2(/* const */ a *uint8, /*const*/ b *uint8) int {
...
func SSE8x8_SSE2(/* const */ a *uint8, /*const*/ b *uint8) int {
  const __m128i zero = _mm_setzero_si128();
  num_pairs := 4;
  __m128i sum = zero;
  int32 tmp[4];
...
func SSE4x4_SSE2(/* const */ a *uint8, /*const*/ b *uint8) int {
  const __m128i zero = _mm_setzero_si128();

  // Load values. Note that we read 8 pixels instead of 4, // but the a/b buffers are over-allocated to that effect.
  const __m128i a0 = _mm_loadl_epi64((/* const */ __*m128i)&a[BPS * 0]);
  const __m128i a1 = _mm_loadl_epi64((/* const */ __*m128i)&a[BPS * 1]);
  const __m128i a2 = _mm_loadl_epi64((/* const */ __*m128i)&a[BPS * 2]);
  const __m128i a3 = _mm_loadl_epi64((/* const */ __*m128i)&a[BPS * 3]);
  const __m128i b0 = _mm_loadl_epi64((/* const */ __*m128i)&b[BPS * 0]);
  const __m128i b1 = _mm_loadl_epi64((/* const */ __*m128i)&b[BPS * 1]);
  const __m128i b2 = _mm_loadl_epi64((/* const */ __*m128i)&b[BPS * 2]);
  const __m128i b3 = _mm_loadl_epi64((/* const */ __*m128i)&b[BPS * 3]);
...
func Mean16x4_SSE2(/* const */ ref *uint8, uint32 dc[4]) {
  ...
  const __m128i a2 = _mm_loadu_si128((/* const */ __*m128i)&ref[BPS * 2]);
  const __m128i a3 = _mm_loadu_si128((/* const */ __*m128i)&ref[BPS * 3]);
  ...
  uint16 tmp[8];
  _mm_storeu_si128((__*m128i)tmp, f0);
  dc[0] = tmp[0] + tmp[1];
  dc[1] = tmp[2] + tmp[3];
  dc[2] = tmp[4] + tmp[5];
  dc[3] = tmp[6] + tmp[7];
...
func TTransform_SSE2(/* const */ inA *uint8, /*const*/ inB *uint8, /*const*/ /* const */ w *uint16) int {
  int32 sum[4];
  ...
  {
    const __m128i inA_0 = _mm_loadl_epi64((/* const */ __*m128i)&inA[BPS * 0]);
    const __m128i inA_1 = _mm_loadl_epi64((/* const */ __*m128i)&inA[BPS * 1]);
    const __m128i inA_2 = _mm_loadl_epi64((/* const */ __*m128i)&inA[BPS * 2]);
    const __m128i inA_3 = _mm_loadl_epi64((/* const */ __*m128i)&inA[BPS * 3]);
    const __m128i inB_0 = _mm_loadl_epi64((/* const */ __*m128i)&inB[BPS * 0]);
    const __m128i inB_1 = _mm_loadl_epi64((/* const */ __*m128i)&inB[BPS * 1]);
    const __m128i inB_2 = _mm_loadl_epi64((/* const */ __*m128i)&inB[BPS * 2]);
  ...
  // Horizontal pass and difference of weighted sums.
  {
    ...
    _mm_storeu_si128((__*m128i)&sum[0], A_b0);
  }
  return sum[0] + sum[1] + sum[2] + sum[3];
}

func Disto4x4_SSE2(/* const */ /* const */ a *uint8, /*const*/ /* const */ b *uint8, /*const*/ /* const */ w *uint16) int {
  diff_sum := TTransform_SSE2(a, b, w);
  return abs(diff_sum) >> 5;
}

func Disto16x16_SSE2(/* const */ /* const */ a *uint8, /*const*/ /* const */ b *uint8, /*const*/ /* const */ w *uint16) int {
  D := 0;
  var x, y int
...
static  int DoQuantizeBlock_SSE2(
    int16 in[16], int16 out[16], /*const*/ /* const */ sharpen *uint16, /*const*/ /* const */ mtx *VP8Matrix) {
  ...
  // position instead of twelfth) and 8th values.
  {
    ...
    _mm_storeu_si128((__*m128i)&out[0], outZ0);
    _mm_storeu_si128((__*m128i)&out[8], outZ8);
...
func QuantizeBlock_SSE2(int16 in[16], int16 out[16], /*const*/ /* const */ mtx *VP8Matrix) int {
  return DoQuantizeBlock_SSE2(in, out, &mtx.sharpen[0], mtx);
}

func QuantizeBlockWHT_SSE2(int16 in[16], int16 out[16], /*const*/ /* const */ mtx *VP8Matrix) int {
  return DoQuantizeBlock_SSE2(in, out, nil, mtx);
}

func Quantize2Blocks_SSE2(int16 in[32], int16 out[32], /*const*/ /* const */ mtx *VP8Matrix) int {
  var nz int
