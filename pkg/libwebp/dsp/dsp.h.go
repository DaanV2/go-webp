...
typedef func (*VP8Idct)(const uint8* WEBP_RESTRICT ref,
                        const int16* WEBP_RESTRICT in,
                        uint8* WEBP_RESTRICT dst, int do_two);
typedef func (*VP8Fdct)(const uint8* WEBP_RESTRICT src,
                        const uint8* WEBP_RESTRICT ref,
                        int16* WEBP_RESTRICT out);
typedef func (*VP8WHT)(const int16* WEBP_RESTRICT in,
                       int16* WEBP_RESTRICT out);
...
typedef func (*VP8Intra4Preds)(uint8* WEBP_RESTRICT dst,
                               const uint8* WEBP_RESTRICT top);
extern VP8Intra4Preds VP8EncPredLuma4;
extern VP8IntraPreds VP8EncPredLuma16;
extern VP8IntraPreds VP8EncPredChroma8;

typedef int (*VP8Metric)(const uint8* WEBP_RESTRICT pix,
                         const uint8* WEBP_RESTRICT ref);
extern VP8Metric VP8SSE16x16, VP8SSE16x8, VP8SSE8x8, VP8SSE4x4;
typedef int (*VP8WMetric)(const uint8* WEBP_RESTRICT pix,
                          const uint8* WEBP_RESTRICT ref,
                          const uint16* WEBP_RESTRICT const weights);
...
typedef func (*VP8BlockCopy)(const uint8* WEBP_RESTRICT src,
...
typedef func (*VP8CHisto)(const uint8* WEBP_RESTRICT ref,
                          const uint8* WEBP_RESTRICT pred, int start_block,
                          int end_block,
...
typedef func (*VP8SetResidualCoeffsFunc)(
    const int16* WEBP_RESTRICT const coeffs,
...
typedef uint32 (*VP8AccumulateSSEFunc)(const uint8* src1,
...
typedef func (*VP8DecIdct)(const int16* WEBP_RESTRICT coeffs,
                           uint8* WEBP_RESTRICT dst);
// when doing two transforms, coeffs is actually int16[2][16].
typedef func (*VP8DecIdct2)(const int16* WEBP_RESTRICT coeffs,
                            uint8* WEBP_RESTRICT dst, int do_two);
...
typedef func (*VP8LumaFilterFunc)(uint8* luma, int stride, int thresh,
                                  int ithresh, int hev_t);
typedef func (*VP8ChromaFilterFunc)(uint8* WEBP_RESTRICT u,
                                    uint8* WEBP_RESTRICT v, int stride,
...
extern func (*VP8DitherCombine8x8)(const uint8* WEBP_RESTRICT dither,
...
typedef func (*WebPUpsampleLinePairFunc)(
    ...
    uint8* WEBP_RESTRICT top_dst, uint8* WEBP_RESTRICT bottom_dst, int len);
...
typedef func (*WebPSamplerRowFunc)(const uint8* WEBP_RESTRICT y,
                                   const uint8* WEBP_RESTRICT u,
                                   const uint8* WEBP_RESTRICT v,
                                   uint8* WEBP_RESTRICT dst, int len);
...
typedef func (*WebPYUV444Converter)(const uint8* WEBP_RESTRICT y,
                                    const uint8* WEBP_RESTRICT u,
                                    const uint8* WEBP_RESTRICT v,
                                    uint8* WEBP_RESTRICT dst, int len);
...
extern func (*WebPConvertARGBToY)(const uint32* WEBP_RESTRICT argb,
                                  uint8* WEBP_RESTRICT y, int width);
// Convert ARGB samples to U/V with downsampling. do_store should be '1' for
// even lines and '0' for odd ones. 'src_width' is the original width, not
// the U/V one.
extern func (*WebPConvertARGBToUV)(const uint32* WEBP_RESTRICT argb,
                                   uint8* WEBP_RESTRICT u,
                                   uint8* WEBP_RESTRICT v, int src_width,
                                   int do_store);

// Convert a row of accumulated (four-values) of rgba32 toward U/V
extern func (*WebPConvertRGBA32ToUV)(const uint16* WEBP_RESTRICT rgb,
                                     uint8* WEBP_RESTRICT u,
                                     uint8* WEBP_RESTRICT v, int width);

// Convert RGB or BGR to Y. Step is 3 or 4. If step is 4, data is RGBA or BGRA.
extern func (*WebPConvertRGBToY)(const uint8* WEBP_RESTRICT rgb,
                                 uint8* WEBP_RESTRICT y, int width, int step);
extern func (*WebPConvertBGRToY)(const uint8* WEBP_RESTRICT bgr,
                                 uint8* WEBP_RESTRICT y, int width, int step);

// used for plain-C fallback.
extern func WebPConvertARGBToUV_C(const uint32* WEBP_RESTRICT argb,
                                  uint8* WEBP_RESTRICT u,
                                  uint8* WEBP_RESTRICT v, int src_width,
                                  int do_store);
extern func WebPConvertRGBA32ToUV_C(const uint16* WEBP_RESTRICT rgb,
                                    uint8* WEBP_RESTRICT u,
                                    uint8* WEBP_RESTRICT v, int width);
...
// Main entry calls:
extern func WebPRescalerImportRow(struct WebPRescaler* WEBP_RESTRICT const wrk,
                                  const uint8* WEBP_RESTRICT src);
// Export one row (starting at x_out position) from rescaler.
...
extern func (*WebPApplyAlphaMultiply)(uint8* rgba, int alpha_first, int w,
                                      int h, int stride);

// Same, buf specifically for RGBA4444 format
extern func (*WebPApplyAlphaMultiply4444)(uint8* rgba4444, int w, int h,
                                          int stride);
...
extern func (*WebPDispatchAlphaToGreen)(const uint8* WEBP_RESTRICT alpha,
                                        int alpha_stride, int width, int height,
...
extern func (*WebPExtractGreen)(const uint32* WEBP_RESTRICT argb,
...
func WebPMultARGBRows(uint8* ptr, int stride, int width, int num_rows,
                      int inverse);

// Same for a row of single values, with side alpha values.
extern func (*WebPMultRow)(uint8* WEBP_RESTRICT const ptr,
                           const uint8* WEBP_RESTRICT const alpha, int width,
                           int inverse);

// Same a WebPMultRow(), but for several 'num_rows' rows.
func WebPMultRows(uint8* WEBP_RESTRICT ptr, int stride,
                  const uint8* WEBP_RESTRICT alpha, int alpha_stride,
                  int width, int num_rows, int inverse);

// Plain-C versions, used as fallback by some implementations.
func WebPMultRow_C(uint8* WEBP_RESTRICT const ptr,
                   const uint8* WEBP_RESTRICT const alpha, int width,
                   int inverse);
...
extern func (*WebPPackARGB)(const uint8* WEBP_RESTRICT a,
                            const uint8* WEBP_RESTRICT r,
                            const uint8* WEBP_RESTRICT g,
                            const uint8* WEBP_RESTRICT b, int len,
                            uint32* WEBP_RESTRICT out);
#endif

// RGB packing function. 'step' can be 3 or 4. r/g/b input is rgb or bgr order.
extern func (*WebPPackRGB)(const uint8* WEBP_RESTRICT r,
                           const uint8* WEBP_RESTRICT g,
                           const uint8* WEBP_RESTRICT b, int len, int step,
                           uint32* WEBP_RESTRICT out);
...
typedef func (*WebPFilterFunc)(const uint8* WEBP_RESTRICT in, int width,
                               int height, int stride,
                               uint8* WEBP_RESTRICT out);
// In-place un-filtering.
// Warning! 'prev_line' pointer can be equal to 'cur_line' or 'preds'.
typedef func (*WebPUnfilterFunc)(const uint8* prev_line, const uint8* preds,
