...
func DoHorizontalFilter_C( /* const */ int []uint8, width, height int, stride int, out []uint8) {
	var preds []uint8 = in
	var row int
...
func DoVerticalFilter_C( /* const */ int []uint8, width, height int, stride int, out []uint8) {
	var preds []uint8 = in
	var row int
...
func GradientPredictor_C(a uint8, b uint8, c uint8) int {
	g := a + b - c
...
func NoneUnfilter_C( /* const */ prev []uint8 /*const*/, int []uint8, out []uint8, width int) {
	_ = prev
	if out != in {
		memcpy(out, in, width*sizeof(*out))
	}
}

func HorizontalUnfilter_C( /* const */ prev []uint8 /*const*/, int []uint8, out []uint8, width int) {
...
func VerticalUnfilter_C( /* const */ prev []uint8 /*const*/, int []uint8, out []uint8, width int) {
	if prev == nil {
...
func GradientUnfilter_C( /* const */ prev []uint8 /*const*/, int []uint8, out []uint8, width int) {
	if prev == nil {
		HorizontalUnfilter_C(nil, in, out, width)
	} else {
		top := prev[0]
...
var WebPUnfilters [4]func(prev []uint8, int []uint8, out []uint8, width int)
