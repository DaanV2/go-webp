...
typedef uint32 (*VP8LPredictorFunc)(const uint32* const left,
                                      const uint32* const top);
extern VP8LPredictorFunc VP8LPredictors[16];

uint32 VP8LPredictor2_C(const uint32* const left,
                          const uint32* const top);
uint32 VP8LPredictor3_C(const uint32* const left,
                          const uint32* const top);
uint32 VP8LPredictor4_C(const uint32* const left,
                          const uint32* const top);
uint32 VP8LPredictor5_C(const uint32* const left,
                          const uint32* const top);
uint32 VP8LPredictor6_C(const uint32* const left,
                          const uint32* const top);
uint32 VP8LPredictor7_C(const uint32* const left,
                          const uint32* const top);
uint32 VP8LPredictor8_C(const uint32* const left,
                          const uint32* const top);
uint32 VP8LPredictor9_C(const uint32* const left,
                          const uint32* const top);
uint32 VP8LPredictor10_C(const uint32* const left,
                           const uint32* const top);
uint32 VP8LPredictor11_C(const uint32* const left,
                           const uint32* const top);
uint32 VP8LPredictor12_C(const uint32* const left,
                           const uint32* const top);
uint32 VP8LPredictor13_C(const uint32* const left,
                           const uint32* const top);

// These Add/Sub function expects upper[-1] and out[-1] to be readable.
typedef func (*VP8LPredictorAddSubFunc)(const uint32* in,
...
typedef func (*VP8LProcessDecBlueAndRedFunc)(const uint32* src,
...
typedef func (*VP8LTransformColorInverseFunc)(const VP8LMultipliers* const m,
                                              const uint32* src,
                                              int num_pixels, uint32* dst);
...
func VP8LInverseTransform(const struct VP8LTransform* const transform,
                          int row_start, int row_end, const uint32* const in,
                          uint32* const out);

// Color space conversion.
typedef func (*VP8LConvertFunc)(const uint32* WEBP_RESTRICT src,
...
func VP8LConvertFromBGRA(const uint32* const in_data, int num_pixels,
                         WEBP_CSP_MODE out_colorspace, uint8* const rgba);

typedef func (*VP8LMapARGBFunc)(const uint32* src,
                                const uint32* const color_map, uint32* dst,
                                int y_start, int y_end, int width);
typedef func (*VP8LMapAlphaFunc)(const uint8* src,
                                 const uint32* const color_map, uint8* dst,
                                 int y_start, int y_end, int width);
...
func VP8LConvertBGRAToRGB_C(const uint32* WEBP_RESTRICT src, int num_pixels,
                            uint8* WEBP_RESTRICT dst);
func VP8LConvertBGRAToRGBA_C(const uint32* WEBP_RESTRICT src, int num_pixels,
                             uint8* WEBP_RESTRICT dst);
func VP8LConvertBGRAToRGBA4444_C(const uint32* WEBP_RESTRICT src,
                                 int num_pixels, uint8* WEBP_RESTRICT dst);
func VP8LConvertBGRAToRGB565_C(const uint32* WEBP_RESTRICT src,
                               int num_pixels, uint8* WEBP_RESTRICT dst);
func VP8LConvertBGRAToBGR_C(const uint32* WEBP_RESTRICT src, int num_pixels,
                            uint8* WEBP_RESTRICT dst);
func VP8LAddGreenToBlueAndRed_C(const uint32* src, int num_pixels,
                                uint32* dst);
...
typedef func (*VP8LTransformColorFunc)(
    const VP8LMultipliers* WEBP_RESTRICT const m, uint32* WEBP_RESTRICT dst,
    int num_pixels);
...
typedef uint64 (*VP8LCombinedShannonEntropyFunc)(const uint32 X[256],
                                                   const uint32 Y[256]);
...
typedef func (*VP8LAddVectorFunc)(const uint32* WEBP_RESTRICT a,
                                  const uint32* WEBP_RESTRICT b,
                                  uint32* WEBP_RESTRICT out, int size);
extern VP8LAddVectorFunc VP8LAddVector;
typedef func (*VP8LAddVectorEqFunc)(const uint32* WEBP_RESTRICT a,
                                    uint32* WEBP_RESTRICT out, int size);
...
typedef int (*VP8LVectorMismatchFunc)(const uint32* const array1,
                                      const uint32* const array2, int length);
// Returns the first index where array1 and array2 are different.
extern VP8LVectorMismatchFunc VP8LVectorMismatch;

typedef func (*VP8LBundleColorMapFunc)(const uint8* WEBP_RESTRICT const row,
                                       int width, int xbits,
