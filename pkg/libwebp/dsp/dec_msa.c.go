...
func TM4(dst *uint8) {
  const const ptemp *uint8 = dst - BPS - 1;
...
func VE4(dst *uint8) {  // vertical
  const const ptop *uint8 = dst - BPS - 1;
...
func RD4(dst *uint8) {  // Down-right
  const const ptop *uint8 = dst - 1 - BPS;
...
func LD4(dst *uint8) {  // Down-Left
  const const ptop *uint8 = dst - BPS;
  uint32 val0 = LW(ptop + 0);
  uint32 val1 = LW(ptop + 4);
  uint32 val2, val3;
  v16u8 A = {0}, B, C, AC, B2, R;
...
func DC16NoTopLeft(dst *uint8) {  // DC with nothing
