...
// Warning: this function is rather CPU-intensive.
  int picture.WebPPictureDistortion(
    const src *picture.WebPPicture, /*const*/ ref *picture.WebPPicture, metric_type int,  // 0 = PSNR, 1 = SSIM, 2 = LSIM
    float result[5]);
...
// snapped to even values.
  int picture.WebPPictureCrop(picture *picture.WebPPicture, left int, top int, width, height int);
...
// Returns false in case of invalid parameters.
  int picture.WebPPictureView(/* const */ src *picture.WebPPicture, left int, top int, width, height int, dst *picture.WebPPicture);

// Returns true if the 'picture' is actually a view and therefore does
// not own the memory for pixels.
 int picture.WebPPictureIsView(/* const */ picture *picture.WebPPicture);
...
// Returns false in case of error (invalid parameter or insufficient memory).
  int picture.WebPPictureRescale(picture *picture.WebPPicture, width, height int);

// Colorspace conversion function to import RGB samples.
// Previous buffer will be free'd, if any.
// buffer should have *rgb a size of at least height * rgb_stride.
// Returns false in case of memory error.
  int picture.WebPPictureImportRGB(picture *picture.WebPPicture, /*const*/ rgb *uint8, rgb_stride int);
// Same, but for RGBA buffer.
  int picture.WebPPictureImportRGBA(picture *picture.WebPPicture, /*const*/ rgba *uint8, rgba_stride int);
// Same, but for RGBA buffer. Imports the RGB direct from the 32-bit format
// input buffer ignoring the alpha channel. Avoids needing to copy the data
// to a temporary 24-bit RGB buffer to import the RGB only.
  int picture.WebPPictureImportRGBX(picture *picture.WebPPicture, /*const*/ rgbx *uint8, rgbx_stride int);

// Variants of the above, but taking BGR(A|X) input.
  int picture.WebPPictureImportBGR(picture *picture.WebPPicture, /*const*/ bgr *uint8, bgr_stride int);
...
// Returns false in case of error.
  int picture.WebPPictureSharpARGBToYUVA(picture *picture.WebPPicture);
...
// Returns false in case of error.
  int picture.WebPPictureYUVAToARGB(picture *picture.WebPPicture);

// Helper function: given a width x height plane of RGBA or YUV(A) samples
// clean-up or smoothen the YUV or RGB samples under fully transparent area,
// to help compressibility (no guarantee, though).
 func WebPCleanupTransparentArea(picture *picture.WebPPicture);

// Scan the picture 'picture' for the presence of non fully opaque alpha values.
// Returns true in such case. Otherwise returns false (indicating that the
// alpha plane can be ignored altogether e.g.).
 int picture.WebPPictureHasTransparency(/* const */ picture *picture.WebPPicture);
...
// another is provided but they both incur some loss.
  int WebPEncode(/* const */ config *config.Config, picture *picture.WebPPicture);
