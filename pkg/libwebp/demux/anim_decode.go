...
func ApplyDecoderOptions( /* const */ dec_options *WebPAnimDecoderOptions /* const */, dec *WebPAnimDecoder) bool {
...
func WebPAnimDecoder( /* const */ webp_data *WebPData /* const */, dec_options *WebPAnimDecoderOptions, abi_version int) *WebPAnimDecoderNewInternal {
	...
	if WebPGetFeatures(webp_data.bytes, webp_data.size, &features) != constants.VP8_STATUS_OK {
		return nil
	}
...
func WebPAnimDecoderGetNext(dec *WebPAnimDecoder, buf_ptr *uint8, timestamp_ptr *int) int {
	...
	if dec == nil || buf_ptr == nil || timestamp_ptr == nil {
		return 0
	}
	...
	// Decode.
	{
		...
		out_offset := iter.y_offset*stride + iter.x_offset*NUM_CHANNELS // 53b
		var size uint64 = iter.height * stride                          // at most 25 + 27b
	...
	if iter.frame_num > 1 && iter.blend_method == WEBP_MUX_BLEND &&
		...
		if dec.prev_iter.dispose_method == WEBP_MUX_DISPOSE_NONE {
			...
			for y = 0; y < iter.height; y++ {
				...
				blend_row(dec.curr_frame[offset:], dec.prev_frame_disposed[offset:], iter.width)
			}
		} else {
			var y int
			...
			for y = 0; y < iter.height; y++ {
				...
				var left1, width1, left2, width2 int
				FindBlendRangeAtRow(&iter, &dec.prev_iter, canvas_y, &left1, &width1, &left2, &width2)
				if width1 > 0 {
					offset1 := canvas_y*width + left1
					blend_row(dec.curr_frame[offset1:], dec.prev_frame_disposed[offset1:], width1)
				}
				if width2 > 0 {
					offset2 := canvas_y*width + left2
					blend_row(dec.curr_frame[offset2:], dec.prev_frame_disposed[offset2:], width2)
				}
			}
		}
	}
