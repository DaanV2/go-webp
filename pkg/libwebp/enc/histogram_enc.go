...
func UpdateDominantCostRange( /* const */ h *VP8LHistogram /*const*/, c *DominantCostRange) {
	if c.literal_max < h.costs[LITERAL] {
		c.literal_max = h.costs[LITERAL]
	}
	if c.literal_min > h.costs[LITERAL] {
		c.literal_min = h.costs[LITERAL]
	}
	if c.red_max < h.costs[RED] {
		c.red_max = h.costs[RED]
	}
	if c.red_min > h.costs[RED] {
		c.red_min = h.costs[RED]
	}
	if c.blue_max < h.costs[BLUE] {
		c.blue_max = h.costs[BLUE]
	}
	if c.blue_min > h.costs[BLUE] {
		c.blue_min = h.costs[BLUE]
	}
}
...
func GetBinIdForEntropy(min uint64, max uint64, val uint64) int {
	...
	if vrange > 0 {
		delta := val - min
		return (int)((NUM_PARTITIONS - 1e-6) * delta / vrange)
	} else {
		return 0
	}
}
...
func HistogramBuild(xsize int, histo_bits int /*const*/, backward_refs *VP8LBackwardRefs /*const*/, image_histo *VP8LHistogramSet) {
	...
	var c VP8LRefsCursor = VP8LRefsCursorInit(backward_refs)
...
func HistogramCopyAndAnalyze( /* const */ orig_histo *VP8LHistogramSet /*const*/, image_histo *VP8LHistogramSet) {
	var i int
	var orig_histograms *VP8LHistogram = orig_histo.histograms
	var histograms *VP8LHistogram = image_histo.histograms
...
func HistogramCombineEntropyBin( /* const */ image_histo *VP8LHistogramSet, cur_combo *VP8LHistogram, num_bins int, int32 combine_cost_factor, low_effort int) {
	var histograms *VP8LHistogram = image_histo.histograms
	var idx int
...
func MyRand( /* const */ seed *uint32) uint32 {
...
func HistoQueueFixPair(int bad_id, good_id int /*const*/, pair *HistogramPair) {
	if pair.idx1 == bad_id {
		pair.idx1 = good_id
	}
	if pair.idx2 == bad_id {
...
// Otherwise returns 0 and the cost is invalid due to early bail-out.
func HistoQueueUpdatePair( /* const */ h *VP8LHistogram1 /*const*/, h *VP8LHistogram2, int64 cost_threshold /*const*/, pair *HistogramPair) int {
...
func HistoQueuePush( /* const */ histo_queue *HistoQueue, histograms *VP8LHistogram, int idx1, int idx2, int64 threshold) int64 {
	var h1 *VP8LHistogram
	var h2 *VP8LHistogram
...
func HistogramCombineStochastic( /* const */ image_histo *VP8LHistogramSet, min_cluster_size int /*const*/, do_greedy *int) int {
	var j, iter int
	seed := 1
	...
	for iter = 0; iter < outer_iters && image_histo.size >= min_cluster_size; iter++ {
		tries_with_no_success++
		if tries_with_no_success >= num_tries_no_success {
			break
		}
		var best_cost int64 = tenary.If(histo_queue.size == 0, 0, histo_queue.queue[0].cost_diff)
		var best_idx1 int = -1
		var best_idx2 int = 1
		...
		for j = 0; image_histo.size >= 2 && j < num_tries; j++ {
			...
			var idx1 uint32 = tmp / (image_histo.size - 1)
			var idx2 uint32 = tmp % (image_histo.size - 1)
			if idx2 >= idx1 {
				idx2++
			}
...
func HistogramRemap( /* const */ in *VP8LHistogramSet /*const*/, out *VP8LHistogramSet /*const*/, symbols *uint32) {
	var i int
	var in_histo *VP8LHistogram = in.histograms
	var out_histo *VP8LHistogram = out.histograms
...
func VP8LGetHistoImageSymbols(xsize int, ysize int /*const*/, refs *VP8LBackwardRefs, quality int, low_effort int, histogram_bits int, cache_bits int /*const*/, image_histo *VP8LHistogramSet /*const*/, tmp_histo *VP8LHistogram /*const*/, histogram_symbols *uint32 /*const*/, pic *picture.Picture, percent_range int /*const*/, percent *int) int {
	histo_xsize := tenary.If(histogram_bits, VP8LSubSampleSize(xsize, histogram_bits), 1)
	histo_ysize := tenary.If(histogram_bits, VP8LSubSampleSize(ysize, histogram_bits), 1)
	image_histo_raw_size := histo_xsize * histo_ysie
	var orig_histo *VP8LHistogramSet = VP8LAllocatezHistogramSet(image_histo_raw_size, cache_bits)
...
func VP8LHistogramNumCodes(palette_code_bits int) int {
	return constants.NUM_LITERAL_CODES + constants.NUM_LENGTH_CODES +
