...
func VP8DefaultProbas(const enc *VP8Encoder) {
  const probas *VP8EncProba = &enc.proba;
...
static int PutI4Mode(const bw *VP8BitWriter, int mode, const const prob *uint8) {
  ...
  do {
    ...
    if (VP8PutBit(bw, (mb.type != 0), 145)) {  // i16x16
      ...
      for (y = 0; y < 4; ++y) {
        int left = preds[-1];
        for (x = 0; x < 4; ++x) {
          const const probas *uint8 = kBModesProba[top_pred[x]][left];
